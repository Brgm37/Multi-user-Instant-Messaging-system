...
class UserServices(
	...
	override fun createUser(user: User): Either<Error, User> {
		val createdUser = userRepo.createUser(user) ?: return failure(UserError.UserAlreadyExists)
	...
	override fun getUser(id: UInt): Either<Error, User> {
		val user = userRepo.findById(id) ?: return failure(UserError.UserNotFound)
		return success(user)
	}
